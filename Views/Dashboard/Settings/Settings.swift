...
struct SettingsView: View {
                        ...
                        HStack {
                            ...
                            Button(action: {
                                ...
                                Image(systemName: "plus.circle.fill")
                                    .font(.system(size: 40)).foregroundColor(Color("AccentCoral"))
                            }
                        }
                        .padding(.horizontal)
    ...
    private func scheduleDailyCheckInNotification() {
        NotificationService.shared.scheduleDailyCheckIn(
            hour: checkInHour, minute: checkInMinute, isAM: checkInHour < 12
        )
    }

    private func scheduleWeeklyReflectionNotification() {
        NotificationService.shared.scheduleWeeklyReflection(
            hour: weeklyReflectionHour, minute: weeklyReflectionMinute, isAM: weeklyReflectionHour < 12
        )
    }
}
...
#Preview {
    SettingsView()
        .modelContainer(for: [User.self, Medication.self, MedicationLog.self])
}
